...
const Picture = ({ veloRef, cover, index, stateRef, title, link }) => {
    const ref = useRef()
    ...
    const onHover = (value) => {
        if (ref.current) ref.current.uHover = value
    ...
    const mouse = (e) => {
        if (ref.current) ref.current.uMouse = e.point
    }
    useFrame(({ clock }) => {
        if (ref.current) ref.current.uTime = clock.getElapsedTime()
        if (meshRef.current && stateRef.current) {
            ...
            <Canvas style={{ position: 'absolute', left: 0, top: 0, zIndex: 2, pointerEvents: 'all' }} camera={{ fov: 8, position: [0, 0, 10] }} >
                ...
                <Suspense fallback={null}>
                    {workSlideShow.map((slide, index) => (
                        <Picture veloRef={veloRef} stateRef={stateRef} key={slide.id} index={index} {...slide} />
                    ))}
                </Suspense>
            </Canvas>
